class InstancedGroup {
    ...
    async initAnimation(uniforms) {
        ...
        const animations = await this.loadJSON(this.animationUrl);//动画数据读取{config(一维数组),animation(二维数组)}//数组的每一个维度对应一个动画
        const boneCount = this.originMesh.skeleton.bones.length;//骨骼个数
        const animationData = animations.animation.flat();//animations.animation将二维数组转换为一维数组
        const animationDataLength = animations.config.reduce((prev, cur) => prev + cur, 0); // sum //animations.animation中元素的总个数
        ...
        console.log(
            ...
            //animationData,
            "boneCount:"+boneCount,
            "animationDataLength:"+animationDataLength,
            "animationTextureLength:"+animationTextureLength,
            "animationCount:"+animations.config.length,
            "FrameCount :"+(animations.config[0] / boneCount / 12)
